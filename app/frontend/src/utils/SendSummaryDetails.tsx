...
export const SendSummaryDetails = async (summaryDetails: SummaryEmailData) => {
  try {
    const response = await fetch(
      `${BASE_URL}/api/conversation/emailSummary/sendEmail`,
      {
        method: "POST",
        headers: {
...
export const getCustomerCommunicationDetails = async (
  ...
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/api/conversation/emailSummary/${threadId}`,
      getRequestOptions
    );

    if (!response.ok) {
      throw new Error(
        `Failed to fetch conversation summary details. Status: ${response.status}`
      );
    }
